  ...
  <svg width="100%" height="98%" id="force">
    <g v-if="edges.length">
      <g v-for="(item, index) in edges" :key="index" class="edge">
        <line
          class="edgeLine"
          stroke-width="3"
          :stroke="item.rel === '亲属关系' ? 'red' : item.rel === '朋友关系' ? 'blue' : 'black' "
        ></line>
    ...
    <g v-if="nodes.length">
      <g v-for="(item, index) in nodes" :key="item.name" class="node">
        <circle
          :r="item.name === $route.query.name ? nodeR :r"
          :fill="`url(#${item.name})`"
          class="nodeCircle"
          @click="addSec(item.name,svg)"
        ></circle>
        <!-- <Pie
          :innerR="item.name === $route.query.name ? nodeR :r"
          :outerR="item.name !== $route.query.name ? 60 :70"
          :i="index"
        ></Pie> -->
        <g>
          <text
        ...
        if (edges.length) {
          edgeLine
            ...
            .attr("stroke-dasharray", function(d) {
              const linkWidth = Math.sqrt(
                (d.source.y - d.target.y) ** 2 + (d.source.x - d.target.x) ** 2
              );
          ...
          edgeTitle
            ...
            .attr("transform", d => {
              ...
              return `rotate(${deg} ${(d.source.x + d.target.x) / 2} ${(d.source
                .y +
                d.target.y) /
                2})`;
            });
